...
public class MainActivity extends Activity {
    private final int ink=0xFF17213C,muted=0xFF596680,accent=0xFF4258DA;
    ...
    private Button button(String value,boolean primary){Button b=new Button(this);b.setText(value);b.setAllCaps(false);b.setTextSize(14);b.setTextColor(primary?Color.WHITE:accent);b.setTypeface(Typeface.create("sans-serif-medium",Typeface.NORMAL));b.setMinHeight(dp(48));b.setMinimumHeight(dp(48));b.setPadding(dp(12),0,dp(12),0);
        Drawable d=glass(18,true);if(primary){GradientDrawable g=new GradientDrawable(GradientDrawable.Orientation.TL_BR,new int[]{0xFF637AF4,0xFF3E4FD1});g.setCornerRadius(dp(18));d=g;}
    ...
    public void onCreate(Bundle state){
        ...
        getWindow().setStatusBarColor(Color.TRANSPARENT);getWindow().setNavigationBarColor(0xFFE7EBFA);
    ...
    private View buildHome(){
        LinearLayout home=column();TextView title=text("Save something great.",25,ink,true);home.addView(title);gap(home,5);home.addView(text("Video or audio. Straight to your phone.",13,muted,false));gap(home,16);
    ...
    private View buildSettings(){LinearLayout l=column();l.addView(text("A few essentials",26,ink,true));gap(l,5);l.addView(text("Simple tools. No account needed.",13,muted,false));gap(l,18);
        ...
        gap(l,16);Button about=button("About & open-source licenses",false);l.addView(about);about.setOnClickListener(v->about());gap(l,10);l.addView(text("POCKET MEDIA 5.0  ·  LIQUID GLASS",11,muted,true));return scrolling(l);}
    ...
    private void about(){
        ...
        content.addView(text("Pocket Media",26,ink,true));gap(content,4);content.addView(text("Version 5.0  ·  Made for your moments",13,muted,false));gap(content,22);
    ...
    private static class LiquidBackground extends View {
        ...
        protected void onDraw(Canvas c){float w=getWidth(),h=getHeight();
            paint.setShader(new LinearGradient(0,0,w,h,new int[]{0xFFF1F4FB,0xFFDAE4FC,0xFFEDEBFA},null,Shader.TileMode.CLAMP));c.drawRect(0,0,w,h,paint);
            paint.setShader(new LinearGradient(0,h*.25f,w,h*.60f,new int[]{0x009CAFFE,0x779CAFFE,0x00FFFFFF},null,Shader.TileMode.CLAMP));
            Path sweep=new Path();sweep.moveTo(-w*.4f,h*.72f);sweep.cubicTo(w*.2f,h*.02f,w*.72f,h*.87f,w*1.3f,h*.22f);sweep.lineTo(w*1.3f,h*.48f);sweep.cubicTo(w*.68f,h*.98f,w*.12f,h*.26f,-w*.4f,h*.96f);sweep.close();c.drawPath(sweep,paint);
            paint.setShader(new LinearGradient(0,0,w,h,new int[]{0x00FFFFFF,0xAAFFFFFF,0x00FFFFFF},null,Shader.TileMode.CLAMP));paint.setStyle(Paint.Style.STROKE);paint.setStrokeWidth(2);c.drawPath(sweep,paint);paint.setStyle(Paint.Style.FILL);
        }
    }
}
